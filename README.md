...
> 매수해야 할 근거와 매도해야 할 근거를 정면으로 맞붙여 판가름하고, 그 결론을 리스크 점검과 과거 판단 기록까지 거쳐 한 부의 투자 보고서로 정리합니다.
...
계좌를 연동하려면 [Upbit → 내 정보 → API 관리](https://upbit.com/mypage/api)에서 키를 발급받아 환경변수로 등록하면 됩니다. 처음에는 조회 권한만 허용하는 것을 권합니다. 분석 기능은 모두 동작하고, 주문 권한은 필요해진 뒤에 추가해도 늦지 않습니다. 키를 채팅이나 코드에 붙여넣지 마세요. 환경변수로만 등록해야 합니다.
...
"KRW-BTC 분석해줘" 한마디면 아래 여덟 단계가 차례로 돌면서, 앞 단계의 결과가 뒷단계의 입력으로 넘어갑니다. 다수결이 아니라 판정이 위에서 아래로 내려오는 구조라서, 근거가 단계마다 쌓입니다.

| | 단계 | 하는 일 |
|--|------|--------|
| 📸 | 스냅샷 | 일봉·분봉 캔들, 티커, 호가, 체결 내역, 지표 JSON, 최근 뉴스, 과거 판단 기록까지 한 번에 수집 |
| 👁 | 시장 분석가 | 추세·변동성·거래량 이상을 몇 문장으로 요약 |
| 🐂 | 강세 논자 | 매수 근거를 스냅샷 수치와 함께 주장 (1라운드) |
| 🐻 | 약세 논자 | 매도·관망 근거를 독립적으로 주장 (1라운드) |
